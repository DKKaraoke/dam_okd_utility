...
class TestOkdMidi(unittest.TestCase):
    ...
    EXTENDED_VALUES: list[tuple[int, bytes]] = [
        (0x000000, b"\x00\x80"),
        (0x00003F, b"\x3f\x80"),
        (0x00103F, b"\x7f\x3f\x80"),
        (0x04103F, b"\x7f\x7f\x3f\x80"),
        (0x04107E, b"\x7f\x7f\x3f\x3f\x80"),
        (0x04207E, b"\x7f\x7f\x3f\x7f\x3f\x80"),
        (0x08207E, b"\x7f\x7f\x3f\x7f\x7f\x3f\x80"),
    ]
    ...
    def test_write_variable_int(
        ...
        for value, buffer in TestOkdMidi.VALUES:
            with self.subTest(value=value, buffer=buffer):
                ...
                written_buffer: bytes = stream.read("bytes")
                self.assertEqual(buffer, written_buffer)
